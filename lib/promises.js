...
// Observe the separation between arguments and control flow functions /
// callbacks.
// ( Promises un-invert the inversion, cleanly separating the input arguments
// from control flow arguments.)
// Observe how the promises which are future results are returned
...
// Observe how contribs_url could be "returned" from the promise, from a
// fulfillment handler which is like a callback in callback approach.
// Also note contribs_url is not a plain value, rather it is a promise and
// its value can be exercised using a fulfillment handler and any errors
// can be retrieved from the reject handler
// Please note "then function always returns a promise."
...
// contribs_url is further processed below and the subsequent request
// to get contributors response is sent.
...
// To process the data in the contribs response, use the fulfillment
// handler again and return the value we are interested in.
...
// how the error propagation happens from the preceding promises in the chain
// to subsequent promises.
